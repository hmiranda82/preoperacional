...
.hist-shell { min-height:100vh;background:var(--bg);font-family:'Barlow',sans-serif;padding-bottom:90px;max-width:560px;margin:0 auto; }
.topbar { background:var(--topbar-bg);padding:calc(env(safe-area-inset-top) + 14px) 18px 12px;display:flex;align-items:center;justify-content:space-between;border-bottom:1px solid var(--topbar-border);position:sticky;top:0;z-index:50; }
...
.state-center { display:flex;flex-direction:column;align-items:center;gap:12px;padding:80px 24px;color:var(--muted);font-family:'Barlow Condensed',sans-serif;font-size:12px;letter-spacing:1.5px; }
.spinner { width:44px;height:44px;border-radius:50%;border:3px solid var(--border-soft);border-top-color:var(--accent-bg);animation:spin 1s linear infinite; }
@keyframes spin{to{transform:rotate(360deg)}}
.empty-state { display:flex;flex-direction:column;align-items:center;gap:12px;padding:60px 28px;text-align:center; }
.es-title { font-family:'Barlow Condensed',sans-serif;font-size:18px;font-weight:800;color:var(--text);letter-spacing:2px;margin:0; }
.es-sub   { font-size:13px;color:var(--muted);margin:0; }
.retry-btn { margin-top: 8px; padding: 10px 16px; border: none; border-radius: 8px; background: var(--accent-bg); color: #fff; font-family: 'Barlow Condensed', sans-serif; font-size: 12px; font-weight: 700; letter-spacing: 1px; cursor: pointer; }
.stats-row { background:var(--surface);padding:16px 18px;display:flex;align-items:center;border-bottom:1px solid var(--border-soft); }
.stat-item { flex:1;display:flex;flex-direction:column;align-items:center;gap:2px; }
.stat-num  { font-family:'Barlow Condensed',sans-serif;font-size: clamp(20px, 5.5vw, 24px);font-weight:900;color:var(--text);line-height:1; }
.stat-num.ok { color:var(--green); }
.stat-num.bad { color:var(--red); }
.stat-lbl  { font-family:'Barlow Condensed',sans-serif;font-size:10px;font-weight:700;letter-spacing:1px;color:var(--muted); }
.stat-sep  { width:1px;height:36px;background:var(--border-soft); }
.hist-list { padding:8px 0; }
.hist-item { display:flex;align-items:center;justify-content:space-between;background:var(--surface);padding:14px 18px;border-bottom:1px solid var(--surface-3);cursor:pointer;transition:background .1s;animation:fadeIn .4s ease both; }
.hist-item:hover { background:var(--surface-2); }
@keyframes fadeIn{from{opacity:0;transform:translateY(6px)}to{opacity:1;transform:translateY(0)}}
.hi-plate { font-family:'Barlow Condensed',sans-serif;font-size:20px;font-weight:900;color:var(--text);letter-spacing:2px;margin-bottom:3px; }
.hi-meta  { display:flex;align-items:center;gap:4px;font-size:12px;color:var(--muted); }
.hi-dot   { opacity:.4; }
...
.st-ok  { background:var(--badge-ok-bg);color:var(--green); }
.st-bad { background:var(--badge-bad-bg);color:var(--red); }
.st-obs { background:var(--badge-obs-bg);color:var(--badge-obs-text); }
.hi-arrow { color:var(--muted); }
/* Sheet */
.sheet-overlay { position:fixed;inset:0;background:rgba(0,0,0,.5);z-index:200;display:flex;align-items:flex-end; }
.sheet { background:var(--sheet-bg);border-radius:20px 20px 0 0;width:100%;max-width:560px;margin:0 auto;max-height:88vh;display:flex;flex-direction:column;animation:sheetUp .3s cubic-bezier(.22,1,.36,1); }
@keyframes sheetUp{from{transform:translateY(100%)}to{transform:translateY(0)}}
.sheet-handle { width:36px;height:4px;border-radius:99px;background:var(--border-soft);margin:12px auto 0;flex-shrink:0; }
.sheet-header { display:flex;align-items:flex-start;justify-content:space-between;padding:14px 18px 12px;border-bottom:1px solid var(--surface-3);flex-shrink:0; }
.sh-eyebrow { font-family:'Barlow Condensed',sans-serif;font-size:10px;font-weight:700;letter-spacing:1.5px;color:var(--muted);margin:0 0 3px; }
.sh-plate { font-family:'Barlow Condensed',sans-serif;font-size: clamp(22px, 6vw, 26px);font-weight:900;color:var(--text);margin:0 0 6px;letter-spacing:2px; }
...
.sh-close { background:var(--surface-2);border:1px solid var(--border-soft);border-radius:8px;width:34px;height:34px;display:flex;align-items:center;justify-content:center;cursor:pointer;color:var(--muted); }
.sheet-meta { padding:12px 18px;border-bottom:1px solid var(--surface-3);flex-shrink:0; }
.sm-row { display:flex;align-items:baseline;gap:14px;padding:5px 0;font-size:13px;color:var(--muted); }
.sm-row span { flex-shrink:0;min-width:70px; }
.sm-row strong { color:var(--text);font-weight:600;word-break:break-word; }
.sheet-body { flex:1;overflow-y:auto;padding:18px 20px 28px; }
.sb-label { font-family:'Barlow Condensed',sans-serif;font-size:11px;font-weight:700;letter-spacing:1.2px;color:var(--muted);margin:0 0 12px; }
.answer-list { display:flex;flex-direction:column;gap:0; }
.answer-item { padding:16px 0 20px;border-bottom:1px solid var(--surface-3); }
.answer-item:last-child { border:none; }
.ai-top { display:flex;align-items:flex-start;justify-content:space-between;gap:12px; }
.ai-q   { font-size:14px;color:var(--text);font-weight:500;margin:0;flex:1;line-height:1.6; }
...
.val-ok  { background:var(--badge-ok-bg);color:var(--green); }
.val-bad { background:var(--badge-bad-bg);color:var(--red); }
.val-obs { background:var(--badge-obs-bg);color:var(--badge-obs-text); }
.ai-obs { font-size:13px;color:var(--text);margin:10px 12px 0;padding:8px 12px;background:var(--surface-2);border-radius:6px;border-left:3px solid var(--border);line-height:1.5; }
